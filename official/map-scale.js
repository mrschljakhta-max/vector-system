(() => {
  ...
  function isMap(map) {
    return !!(map && typeof map.on === 'function' && typeof map.distance === 'function' && typeof map.getCenter === 'function');
  }
  ...
  function ensureScaleElement() {
    ...
    style.textContent = `
      #vectorScaleIndicator{
        ...
        min-width:170px;
        padding:10px 14px;
        border:1px solid rgba(215,130,25,.65);
        background:rgba(18,22,25,.86);
        color:#f4f2ee;
      ...
      #workspace.vector-workspace.is-open ~ #vectorScaleIndicator,
      body:has(#workspace.vector-workspace.is-open) #vectorScaleIndicator{display:block;}
      #vectorScaleIndicator .scale-label{display:block;color:#d78219;text-transform:uppercase;font-weight:700;font-size:12px;line-height:1;margin-bottom:4px;}
      #vectorScaleIndicator strong{display:block;font-size:22px;line-height:1.05;color:#fff;}
    `;
  ...
  function estimateScale(map) {
    ...
    const cssDpi = 96;
    const metersPerInch = metersPerPixel * cssDpi;
    const scale = metersPerInch / 0.0254;
    return roundToTenThousand(scale);
  }
  ...
  window.addEventListener('load', () => {
    ensureScaleElement();
    const timer = setInterval(() => { bind(); update(); }, 700);
    setTimeout(() => clearInterval(timer), 12000);
  });
  document.addEventListener('click', () => setTimeout(() => { bind(); update(); }, 250));
})();