(() => {
  ...
  function ensureScaleElement() {
    ...
    style.textContent = `#vectorScaleIndicator{position:fixed;left:96px;top:24px;bottom:auto;z-index:8500;min-width:170px;padding:10px 14px;border:1px solid rgba(215,130,25,.65);background:rgba(18,22,25,.86);color:#f4f2ee;box-shadow:0 18px 40px rgba(0,0,0,.35);clip-path:polygon(10px 0,100% 0,100% calc(100% - 10px),calc(100% - 10px) 100%,0 100%,0 10px);font-family:Rajdhani,Arial,sans-serif;letter-spacing:.08em;pointer-events:none;display:none}body:has(#workspace.vector-workspace.is-open) #vectorScaleIndicator{display:block}#vectorScaleIndicator .scale-label{display:block;color:#d78219;text-transform:uppercase;font-weight:700;font-size:12px;line-height:1;margin-bottom:4px}#vectorScaleIndicator strong{display:block;font-size:22px;line-height:1.05;color:#fff}@media(max-width:760px){#vectorScaleIndicator{left:88px;top:16px;min-width:150px;padding:8px 12px}#vectorScaleIndicator strong{font-size:19px}}`;
