(() => {
  ...
  const LAYER_STYLES = {
    summary:{color:'#8B5CF6',fillColor:'#8B5CF6'}, uav:{color:'#3B82F6',fillColor:'#3B82F6'}, stations:{color:'#22C55E',fillColor:'#22C55E'},
  ...
  const DATASETS = [
    ...
    { key:'routes', category:'objects', title:'Шляхи', desc:'маршрутизація по дорогах між НП', table:'map_routes', line:true, columns:['route_code','route_name','route_purpose','route_status','from_name','from_lat','from_lon','to_name','to_lat','to_lon','note'] },
  ...
  async function applyDataSelection(){
    ...
    for(const d of chosen.filter(x=>x.point||x.line)){
      ...
      for(const r of (data||[]).filter(row=>!d.filter||d.filter(row))){
        if(d.line){ status('Прокладаю шляхи по дорогах: '+(added+1)+'...'); const line=await makeRoadRouteLine(r); if(line){ line.addTo(layerGroup); added++; } continue; }
        const lat=Number(r[d.lat]), lon=Number(r[d.lon]); if(!Number.isFinite(lat)||!Number.isFinite(lon)) continue;
  ...
  async function makeRoadRouteLine(r){
    ...
    return window.L.polyline(lineCoords, options).bindPopup(routePopup(r, !!coords));
  }
  ...
  function routePopup(r, routed){ return '<b>'+esc(r.route_name)+'</b><br><span>Шлях</span><hr>Звідки: <b>'+esc(r.from_name)+'</b><br>Куди: <b>'+esc(r.to_name)+'</b><br>Призначення: <b>'+esc(r.route_purpose||'—')+'</b><br>Статус: '+esc(r.route_status||'—')+'<br>Геометрія: <b>'+(routed?'по дорогах':'fallback, потребує уточнення')+'</b>'; }
