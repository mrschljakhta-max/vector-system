...
function loadExportStable(){
  ...
  setTimeout(syncMapToolVisibility,100);
}
...
window.enterVector=function(){
  ...
  showSection('map');
  setTimeout(()=>{ensureMapDataButton();syncMapToolVisibility();try{window.vectorMap?.invalidateSize?.()}catch{}},250);
};
window.logoutVector=function(){
  workspace?.classList.remove('is-open');workspace?.setAttribute('aria-hidden','true');
  landingScreen?.removeAttribute('hidden');document.querySelector('.landing-bg')?.removeAttribute('hidden');
  localStorage.removeItem('vector-user-session');syncMapToolVisibility();
};
function renderFileList(){
  if(!fileList)return;
  let files=[];try{files=JSON.parse(localStorage.getItem('vector-reference-files')||'[]')}catch{}
  if(!files.length){fileList.textContent='Файли ще не додані.';return}
  fileList.innerHTML=files.map(f=>`<div class="file-entry"><strong>${String(f.name||'')}</strong><span>${String(f.type||'')}</span><span>${f.importedTableId?'імпортована таблиця':''}</span></div>`).join('');
}
...
loginForm?.addEventListener('submit',e=>{e.preventDefault();window.enterVector()});
registerForm?.addEventListener('submit',e=>{e.preventDefault();alert('Заявку зафіксовано локально.')});
document.querySelectorAll('.nav__item').forEach(btn=>btn.addEventListener('click',()=>showSection(btn.dataset.section)));
clearFiles?.addEventListener('click',()=>{localStorage.removeItem('vector-reference-files');renderFileList()});
renderFileList();
setInterval(syncMapToolVisibility,250);
document.addEventListener('click',()=>setTimeout(syncMapToolVisibility,40));