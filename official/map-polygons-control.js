(() => {
  ...
  const OUTLINE_STYLE={color:'#16A34A',weight:5.2,opacity:1,vectorLayerKey:'polygons',lineCap:'round',lineJoin:'round',pane:'vectorOutlinePane'};
  const HALO_STYLE={color:'#FFFFFF',weight:2.0,opacity:.92,vectorLayerKey:'polygons',lineCap:'round',lineJoin:'round',pane:'vectorOutlinePane'};
  ...
  function ensureOutlinePane(map){let p=map.getPane('vectorOutlinePane');if(!p){p=map.createPane('vectorOutlinePane');p.style.zIndex=680;p.style.pointerEvents='none'}return p;}
  ...
  function drawOutlineLines(map,feature,group){ensureOutlinePane(map);ringsFromFeature(feature).forEach(r=>{window.L.polyline(r,HALO_STYLE).addTo(group).bringToFront?.();window.L.polyline(r,OUTLINE_STYLE).addTo(group).bringToFront?.();});}
  ...
  async function applyCoveragePolygon(){
    ...
    status('Формую заливку і незалежний зовнішній контур...'); const group=ensureGroup(map); hideStationRadii();
    try{
