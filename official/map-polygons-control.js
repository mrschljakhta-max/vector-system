(() => {
  ...
  const MERGED_STYLE={color:'#22C55E',fillColor:'#22C55E',weight:1,opacity:.45,fillOpacity:.006,vectorLayerKey:'polygons'};
  ...
  function forceSvgTransparency(layer){
    setTimeout(()=>{try{layer.eachLayer?.(l=>{const el=l.getElement?.(); if(el){el.setAttribute('fill-opacity',String(MERGED_STYLE.fillOpacity));el.setAttribute('stroke-opacity',String(MERGED_STYLE.opacity));el.style.fillOpacity=String(MERGED_STYLE.fillOpacity);el.style.opacity='1';}})}catch{}},80);
  }
  ...
  async function applyCoveragePolygon(){
    ...
    status('Групую перетини і формую максимально прозорі полігони...'); const group=ensureGroup(map); hideStationRadii();
    try{
      ...
      for(const comp of components){
        ...
        layer.addTo(group); forceSvgTransparency(layer); mergedGroups++;
      }
