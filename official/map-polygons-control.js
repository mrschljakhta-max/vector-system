(() => {
  ...
  const RADIUS_KM=15;
  const SINGLE_STYLE={color:'#15803D',fillColor:'#22C55E',weight:1.25,opacity:.62,fillOpacity:0,vectorLayerKey:'polygons',lineCap:'round',lineJoin:'round'};
  const OUTLINE_STYLE={color:'#15803D',weight:1.7,opacity:.82,vectorLayerKey:'polygons',lineCap:'round',lineJoin:'round',pane:'vectorCoveragePane'};
  const HALO_STYLE={color:'#FFFFFF',weight:3.0,opacity:.22,vectorLayerKey:'polygons',lineCap:'round',lineJoin:'round',pane:'vectorCoveragePane'};
  ...
  function isStationRadiusLayer(layer){if(!layer||typeof layer.getRadius!=='function'||typeof layer.setStyle!=='function')return false;const radius=Number(layer.getRadius());if(!Number.isFinite(radius)||radius<1000)return false;const o=layer.options||{};const color=String(o.color||o.fillColor||'').toUpperCase();return color==='#22C55E'||color==='#15803D'||o.vectorLayerKey==='stationRadius'}
  function rememberOriginal(layer){ if(!layer||!layer.setStyle) return; if(!layer.__vectorPolygonOriginalStyle){ const o=layer.options||{}; layer.__vectorPolygonOriginalStyle={opacity:o.opacity??1,fillOpacity:o.fillOpacity??0.7,weight:o.weight??2,color:o.color,fillColor:o.fillColor}; } }
  function setSuppressed(layer,on){if(!layer)return;layer.__vectorStationRadiusSuppressedByCoverage=on;layer.__vectorStationRadiusSuppressedByPolygon=on;}
  function collectStationRadiusLayers(){const layers=new Set();const registrySet=window.vectorLayerRegistry?.stationRadius;if(registrySet) registrySet.forEach(layer=>layers.add(layer));const map=getMap();if(map?.eachLayer){map.eachLayer(layer=>{if(isStationRadiusLayer(layer)) layers.add(layer); if(typeof layer.eachLayer==='function') layer.eachLayer(child=>{if(isStationRadiusLayer(child)) layers.add(child);});});}return layers;}
  function hideStationRadii(){ collectStationRadiusLayers().forEach(layer=>{rememberOriginal(layer);setSuppressed(layer,true);layer.setStyle?.({opacity:0,fillOpacity:0,weight:0});const el=layer.getElement?.();if(el){el.style.pointerEvents='none';el.style.display='none';}}); }
  function restoreStationRadii(){ collectStationRadiusLayers().forEach(layer=>{setSuppressed(layer,false);if(layer.__vectorPolygonOriginalStyle)layer.setStyle?.(layer.__vectorPolygonOriginalStyle);const el=layer.getElement?.();if(el){el.style.pointerEvents='';el.style.display='';}}); }
  ...
  function ensurePanel(){ let p=document.querySelector('#vectorPolygonPanel'); if(p) return p; p=document.createElement('section'); p.id='vectorPolygonPanel'; p.innerHTML='<div class="vpp-head"><div><b>Контур</b><span>Заливку вимкнено. Відображається тільки тонка зовнішня межа покриття.</span></div><button type="button" id="vppClose">×</button></div><div id="vppStatus">Готово.</div><div id="vppList"><label class="vpp-row"><input id="coverageToggle" type="checkbox" checked><span class="vpp-title"><i class="vpp-dot"></i>Контур покриття 15 км</span><span class="vpp-count" id="coverageCount">—</span></label></div><div class="vpp-actions"><button id="vppRefresh" type="button">Оновити</button><button id="vppApply" type="button">Застосувати</button></div>'; document.body.appendChild(p); const st=document.createElement('style'); st.id='vectorPolygonControlStyles'; st.textContent=`
  ...
  async function applyCoveragePolygon(){
    ...
    const enabled=document.querySelector('#coverageToggle')?.checked!==false;
    const group=ensureGroup(map);
    if(!enabled){ group.clearLayers(); coverageActive=false; restoreStationRadii(); document.querySelector('#openPolygonControl')?.classList.remove('is-active'); status('Контур покриття вимкнено.'); return; }
    if(!stations.length) await loadStations(); if(!stations.length){status('Немає станцій із координатами.'); return;}
    coverageActive=true; document.querySelector('#openPolygonControl')?.classList.add('is-active'); status('Формую тонкий контур без заливки...'); hideStationRadii(); ensureCoveragePane(map);
    try{
    ...
    }catch(e){ console.warn(e); coverageActive=false; restoreStationRadii(); document.querySelector('#openPolygonControl')?.classList.remove('is-active'); status('Не вдалося сформувати контур. Окремі радіуси повернено.'); }
  }

  window.vectorCoverageControl={apply:applyCoveragePolygon,hideStationRadii,restoreStationRadii,isActive:()=>coverageActive};
  function boot(){mountButton();ensurePanel();if(coverageActive)hideStationRadii();}
  window.addEventListener('load',()=>{boot();setInterval(boot,1200)}); document.addEventListener('click',()=>setTimeout(boot,120));
})();