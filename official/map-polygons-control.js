(() => {
  ...
  async function applyCoveragePolygon(){
    ...
    status('Групую перетини і формую полігони...'); const group=ensureGroup(map); hideStationRadii();
    try{
      ...
      for(const comp of components){
        ...
        if(comp.length===1){ const s=comp[0]; window.L.circle([Number(s.lat),Number(s.lon)],{radius:kmRadius(s)*1000,...COVERAGE_STYLE}).bindPopup('<b>'+String(s.station_name||s.station_code||'Станція')+'</b><br>Окремий радіус '+kmRadius(s)+' км').addTo(group); singleGroups++; continue; }
        ...
        window.L.geoJSON(merged,{style:COVERAGE_STYLE}).bindPopup('<b>Злитий полігон покриття</b><br>Станцій у групі: '+comp.length+'<br>Радіуси перетинаються').addTo(group); mergedGroups++;
      }
