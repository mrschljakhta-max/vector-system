(() => {
  ...
  function mount() {
    ...
    if (!panel) {
      ...
      panel.innerHTML = `
        ...
        <div class="pq-actions" id="pqActions" hidden><button class="pq-btn" id="pqClear" type="button">Скинути</button><button class="pq-btn pq-btn--primary" id="pqSave" type="button">Зберегти схему</button></div>`;
  ...
  function bind() {
    ...
    if (save && !save.dataset.bound) { save.dataset.bound = '1'; save.addEventListener('click', saveSchema); }
  }
  ...
  function readTable(file) {
    ...
    return ext === 'csv' ? readCsv(file) : readExcel(file);
  }
  ...
  function readExcel(file) {
    return new Promise((resolve, reject) => {
      if (!window.XLSX) { reject(new Error('Бібліотека XLSX не завантажилась. Онови сторінку через Ctrl+F5.')); return; }
      const reader = new FileReader();
  ...
  function autoMap(headers) {
    ...
    headers.forEach((header) => {
      ...
      else if (/(num|кільк|числ|number)/.test(h)) map[header] = 'number';
      else map[header] = '';
    });
    return map;
  }
  ...
  function getSaved() { try { return JSON.parse(localStorage.getItem(storageKey) || '[]'); } catch { return []; } }
  function reset() {
    current = null; setStatus('Файл ще не завантажено.');
    const wrap = document.querySelector('#pqTableWrap'); if (wrap) wrap.innerHTML = '<div class="pq-empty">Після завантаження тут з’явиться попередній перегляд таблиці.</div>';
    const summary = document.querySelector('#pqSummary'); if (summary) summary.hidden = true;
    const actions = document.querySelector('#pqActions'); if (actions) actions.hidden = true;
  }
